...
plt.scatter(
    hexagon_centers[:, 0], hexagon_centers[:, 1], color="red", s=10
)  # Red dots for hexagon centers
plt.scatter(
    powiat_centers[:, 0], powiat_centers[:, 1], color="green", s=50
)  # Green dots for powiat centers
plt.show()
...
for tile_count in sorted(powiaty_df["tiles"].unique(), reverse=True):
    ...
    with tqdm(total=len(group)) as pbar:  # Progress bar for the while loop
        while not group.empty:
            ...
            distances = {
                powiat: distance.cdist([center], remaining_hexagons)
                for powiat, center in zip(
                    group["teryt"], group[["easting", "northing"]].to_numpy()
                )
            }
            ...
            sorted_hexes = sorted(
                remaining_hexagons,
                key=lambda h: distance.euclidean(
                    h,
                    group.loc[
                        group["teryt"] == farthest_closest, ["easting", "northing"]
                    ].values[0],
                ),
            )
            ...
            for hexagon in assign_to_powiat:
                assigned_hexagons.append(
                    {
                        "name": group.loc[
                            group["teryt"] == farthest_closest, "name"
                        ].values[0],
                        "teryt": group.loc[
                            group["teryt"] == farthest_closest, "teryt"
                        ].values[0],
                        "powiat_easting": group.loc[
                            group["teryt"] == farthest_closest, "easting"
                        ].values[0],
                        "powiat_northing": group.loc[
                            group["teryt"] == farthest_closest, "northing"
                        ].values[0],
            ...
            remaining_hexagons = [
                h for h in remaining_hexagons if h not in assign_to_powiat
            ]

            # Remove powiat from the group
            group = group[group["teryt"] != farthest_closest]
            assigned_powiaty = assigned_powiaty + 1

            pbar.update(1)  # Update progress bar

# Save the output as a CSV file

print(f"Assigned {assigned_powiaty} powiaty")
